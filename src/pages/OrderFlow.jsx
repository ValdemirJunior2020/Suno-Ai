...
export default function OrderFlow() {
  ...
  return (
    <>
      {/* Header row (brand + flags) */}
      <div className="header">
        <div className="brand">
          <h1>{t("appName")}</h1>
          <p>{t("tagline")}</p>
          <div className="notice small">
            Env check — PayPal mode: <b>{(import.meta.env.VITE_PAYPAL_ENV || "auto").toUpperCase()}</b>
          </div>
        </div>

        <div className="lang langRight">
          <button className={i18n.language === "en" ? "flagBtn active" : "flagBtn"} onClick={() => switchLang("en")}>
            <img className="flag" src="/flags/us.svg" alt="English (US)" />
          </button>
          <button className={i18n.language === "es" ? "flagBtn active" : "flagBtn"} onClick={() => switchLang("es")}>
            <img className="flag" src="/flags/es.svg" alt="Español (España)" />
          </button>
          <button className={i18n.language === "fr" ? "flagBtn active" : "flagBtn"} onClick={() => switchLang("fr")}>
            <img className="flag" src="/flags/fr.svg" alt="Français (France)" />
          </button>
          <button className={i18n.language === "pt" ? "flagBtn active" : "flagBtn"} onClick={() => switchLang("pt")}>
            <img className="flag" src="/flags/br.svg" alt="Português (Brasil)" />
          </button>
        </div>
      </div>
          ...
          <>
            ...
            <PayPalButton
              ...
              onSuccess={async ({ orderId, payerEmail }) => {
                try {
                  await saveToSheet({ orderId, payerEmail, status: "PAID" });
                  navigate("/success", {
                    state: {
                      orderId,
                      payerEmail,
